...
class DirectComputeKernel {
 ...
 private:
  ...
  using BindingData = std::tuple<std::vector<D3D12_DESCRIPTOR_RANGE1>,
                                 std::unordered_map<std::string, DirectComputeKernel::BindPoint>>;

  void CreateRootSignatureAndBindingMap(ComPtr<ID3D12ShaderReflection> _reflection,
                                        std::unordered_map<std::string, BindPoint>& _bindpoints,
 ...
 private:
  ...
  std::unordered_map<std::string, BindPoint> _bindpoints;
  std::vector<BindPoint> _bindpointlist;
  ...
  void binding_buffers(DirectXDevice* _dxdev, ComPtr<ID3D12DescriptorHeap> heap,
                       const std::unordered_map<std::string, BindPoint>& _bindpoints,
